...
const getAllInOneCall = async () => {
	const { currentLoad, mem, networkStats, fsStats }: GetAllResult = await get({
		currentLoad: "currentLoad",
		mem: "active",
		networkStats: "rx_sec,tx_sec",
		fsStats: "rx_sec,wx_sec",
	});
	const currentLoadText = `$(pulse)${currentLoad?.currentLoad?.toFixed(2)}%`;
	const memText = `$(server)${prettyBytes(mem.active)}`;
	const networkStatsText = `$(cloud-download)${prettyBytes(
		networkStats?.[0]?.rx_sec ?? 0,
	)}$(cloud-upload)${prettyBytes(networkStats?.[0]?.tx_sec ?? 0)}`;
	const fsStatsText = `$(log-in)${prettyBytes(
		fsStats?.wx_sec ?? 0,
	)}$(log-out)${prettyBytes(fsStats?.rx_sec ?? 0)}`;
